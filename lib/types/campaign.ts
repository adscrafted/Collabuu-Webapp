...
export const CAMPAIGN_TYPE_OPTIONS: CampaignTypeOption[] = [
  ...
  {
    type: CampaignType.PAY_PER_POST,
    icon: Camera,
    title: 'Pay Per Post',
    description: 'Pay for social media posts',
    exampleUseCase: 'Ideal for influencer marketing campaigns focused on content creation',
    pricingModel: 'Variable - Set credits per post',
  },
];
...
export interface CampaignStats {
  ...
  visitsCount: number;
  creditsSpent: number;
  acceptedInfluencersCount?: number; // Number of accepted influencers
}
...
export interface InfluencerApplication {
  ...
  applicationType?: 'application' | 'invitation';
  appliedAt: string;
  reviewedAt?: string;
  reviewerNotes?: string;
  visitCount?: number; // Only for accepted/approved applications
  socialMediaHandles?: {
    instagram?: string;
    tiktok?: string;
    youtube?: string;
